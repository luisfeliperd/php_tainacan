...
$responsaveis_id = $metadataRepo->fetch(['name'=>'ID Responsável', 'collection_id'=>$responsaveis_collection->get_id()], 'OBJECT');
$responsaveis_id = $responsaveis_id[0];

$instituicoes_collection = $collectionsRepo->fetch(['name'=>'Instituições'], 'OBJECT');
$instituicoes_collection = $instituicoes_collection[0];

$instituicoes_id = $metadataRepo->fetch(['name'=>'ID Instituição', 'collection_id'=>$instituicoes_collection->get_id()], 'OBJECT');
$instituicoes_id = $instituicoes_id[0];
...
$collection->set_name('Portal de Revistas');
$collection->set_status('publish');
$collection->set_description('Coleção com informações do portal de revistas.');

$cont = 0;
if ($collection->validate()) {
	$insertedCollection = $collectionsRepo->insert($collection);

	if (($handle = fopen("portal_revistas_metadados.csv", "r")) == TRUE) {
